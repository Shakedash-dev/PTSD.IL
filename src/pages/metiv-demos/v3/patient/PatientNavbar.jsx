...
import { usePtsdIlPathname, VersionLinks } from './PtsdIlRouting';
import { HelpNow } from '../components/Chrome';

// Metiv demo (V3): a copy of src/components/Navbar.jsx for the patient area.
// Changes only: the Metiv logo replaces the PTSD.IL wordmark (and leads to the
// Metiv home page), the site's "עזרה עכשיו" button joins the controls, and
// paths are read through usePtsdIlPathname(). The user type is also synced
// here, because UserTypeProvider matches exact root paths.

const METIV_LOGO = `${import.meta.env.BASE_URL || '/'}images/metiv-demo/metiv-logo.png`;
...
const USER_TYPE_KEY = 'ptsd_il_user_type';
...
export default function Navbar() {
  ...
  const navItems = selectLinks(pathname, userType);

  useEffect(() => {
    const matched = PATH_TO_TYPE[pathname];
    if (matched && matched !== userType) {
      setUserType(matched);
      try { localStorage.setItem(USER_TYPE_KEY, matched); } catch { /* storage unavailable */ }
    }
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border">
      <nav className="max-w-5xl mx-auto px-5 sm:px-6 h-16 flex items-center justify-between gap-4">
        {/* Metiv logo (replaces the PTSD.IL wordmark) */}
        <VersionLinks>
          <DemoLink to="/" className="flex items-center flex-shrink-0">
            <img src={METIV_LOGO} alt="מטיב - המרכז הישראלי לטיפול בפסיכוטראומה" className="h-9 w-auto" />
          </DemoLink>
        </VersionLinks>
