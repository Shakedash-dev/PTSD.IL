...
import { tx } from '../copy';
import { getQuestionnaire } from '../staticData';
import { ConsultationBlock, CrisisLines } from '../components';

// `question` is the snapshot question object: { text, options: [{answer, score, order}] }.
...
export default function Questionnaire() {
  ...
  const title = q?.name || tx('questionnaire_title');
  const subtitle = q?.description || tx('questionnaire_intro');
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-8 pb-4 text-center">
        <p className="text-xs text-muted-foreground/70">
          {tx('questionnaire_anonymous_note')}
        </p>
      </div>
      ...
      {!q ? (
        <div className="max-w-3xl mx-auto px-4 sm:px-6 pb-16">
          <div className="rounded-super p-8 text-center bg-card border border-warning/30">
            ...
            <p className="text-muted-foreground">שגיאה בטעינת השאלון. נסו לרענן.</p>
          </div>
        </div>
      ) : result === null ? (
        <div className="max-w-3xl mx-auto px-4 sm:px-6 pb-16">
          ...
          <div className="mt-10 text-center">
            ...
            {answered < TOTAL && (
              <p className="text-sm text-muted-foreground mt-2">
                {`${TOTAL - answered} ${tx('questions_remaining_suffix')}`}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 pb-16">
          <div className={`rounded-super p-8 sm:p-10 text-center shadow-atmospheric-lg border ${
            ...
            <h2 className="text-2xl font-heading font-semibold text-foreground mb-4">
              {tx(isHigh ? 'result_high_title' : 'result_low_title')}
            </h2>
            <p className="text-muted-foreground leading-relaxed mb-8">
              {tx(isHigh ? 'result_high_text' : 'result_low_text')}
            </p>

            <div className="mb-8">
              <div className="h-3 rounded-full overflow-hidden bg-gradient-to-l from-success via-warning to-destructive">
                <div
              ...
              <div className="flex justify-between text-xs text-muted-foreground mt-1">
                <span>{tx('scale_mild')}</span>
                <span>{tx('scale_significant')}</span>
              </div>
            </div>

            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <DemoLink
                to={isHigh ? ROUTES.selfHelp : ROUTES.calming}
              ...
              >
                {tx(isHigh ? 'go_to_self_help' : 'go_to_calming')}
          ...
          {/* Metiv addition 8: consultation offer, next to the crisis lines */}
          <div className="mt-8 space-y-4 text-start">
            <ConsultationBlock />
            <CrisisLines className="justify-center" />
          </div>
        </div>
      )}
    </div>
...
// Hebrew sectioned render: slices the flat question list into the overlay's
